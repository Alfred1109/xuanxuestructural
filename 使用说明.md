...
- **前端界面**: http://localhost:8003/index.html
...
如果8002端口被占用，建议先运行 `./stop.sh`。如果仍需改端口，可以修改：
...
同时修改前端统一配置文件 `xuanxue-web/frontend/config.js` 中的API地址：
```javascript
window.APP_CONFIG = { API_BASE_URL: 'http://localhost:8002' };
```
...
### 六爻占卜
```bash
curl -X POST "http://localhost:8002/api/divination/liuyao" \
  -H "Content-Type: application/json" \
  -d '{
    "question": "今天运势如何"
  }'
...
### 奇门遁甲
```bash
curl -X POST "http://localhost:8002/api/divination/qimen" \
  -H "Content-Type: application/json" \
  -d '{
    "year": 2026,
    "month": 2,
...
### 2. 前端无法连接后端
- 确保后端服务已启动
- 确保前端通过 `http://localhost:8003` 访问，而不是直接双击 HTML
- 检查浏览器控制台是否有CORS错误
- 确认API地址是否正确
